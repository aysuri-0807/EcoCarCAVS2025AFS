...
def test_minimum_following_distance_requirement(scenario):
    ...
    if len(failure_indices) > 0: 
        ...
        pytest.fail(
            ...
            f"{failure_df.to_string(index=True)}")
...
def test_speed_error_requirement(scenario):
    ...
    ego_lead_speed_error = df["speed_error"]
    ...
    # 2. Converting to m / 0.01s and taking roughly 1/4 of that value (0.0268224 ~= 4*0.0067)
    # *Value is currently arbitrary and only based on assumed accelerations of simple driving
    acceleration_threshold = 0.0067
